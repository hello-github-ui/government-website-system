...
首次部署执行一次：
```bash
mvn -q compile exec:java -Dexec.mainClass=com.gov.gows.init.DbInitializer
```
...
java -jar target/gov-website-system.jar --server.port=8080
```
...
| 检查项 | 命令 / 地址 |
|---|---|
| 服务存活 | `curl http://localhost:8080/` |
| 后台登录 | http://服务器IP:8080/admin/login （admin / Admin@123456） |
| 健康日志 | 控制台输出 / logs |
